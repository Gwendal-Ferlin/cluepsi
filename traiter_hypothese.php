...
if ($id_joueur > 0 && $id_partie > 0 && $prof > 0 && $salle > 0 && $matiere > 0) {
    ...
    // Remettre `dans_piece` à 0 après avoir donné la réponse
    $sql_update_dans_piece = "UPDATE joueur SET dans_piece = 0 WHERE id_joueur = ?";
    $stmt_update = $conn->prepare($sql_update_dans_piece);
    $stmt_update->bind_param("i", $id_joueur);
    $stmt_update->execute();

    if ($stmt_update->affected_rows <= 0) {
        echo "Erreur lors de la mise à jour du statut de la pièce.";
    }
